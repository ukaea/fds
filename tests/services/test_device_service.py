...
def test_create_device(session: Session):
    ...
    device = service.create(device_create)
...
def test_get_devices(session: Session):
    service = DeviceService(session)
    device1 = service.create(DeviceCreate(name="Device A", type="Type X"))
    device2 = service.create(DeviceCreate(name="Device B", type="Type Y"))
...
def test_get_devices_with_limit_and_offset(session: Session):
    service = DeviceService(session)
    for i in range(10):
        service.create(DeviceCreate(name=f"Device {i}", type="Type A"))
...
def test_delete_device(session: Session):
    ...
    device_deleted = service.delete(created_device.id)
...
def test_delete_device_not_found(session: Session):
    service = DeviceService(session)
    device_deleted = service.delete(999)
