"""Reference-calibration resolution and staged-chain validation.

The resolution selectors, reference integrity, and shot-edit/delete protection
are the shared engine, exercised by ``test_geometry_service``. These tests cover
only what calibration adds: ordered chains and per-``(role, stage)`` non-overlap.
"""

import pytest

from app.models.coverage import Coverage, ShotRange
...
def test_resolve_single_stage(calibration, make_cal_version, get_shot):
    version = make_cal_version("gain", ["gain"], Coverage(shots=["150"]), stage=1)
...
def test_resolve_chain_ordered_by_stage(calibration, make_cal_version, get_shot):
    ...
    absolute = make_cal_version(
        "absolute", ["signal"], Coverage(shots=["150"]), stage=3
    )
    gain = make_cal_version("gain", ["signal"], Coverage(shots=["150"]), stage=1)
    wavelength = make_cal_version(
        "wavelength", ["signal"], Coverage(shots=["150"]), stage=2
    )
...
def test_same_role_different_stage_overlap_allowed(make_cal_version):
    ...
    make_cal_version("gain", ["signal"], Coverage(shots=["150"]), stage=1)
    make_cal_version("absolute", ["signal"], Coverage(shots=["150"]), stage=2)


def test_same_role_same_stage_overlap_rejected(make_cal_version):
    make_cal_version("a", ["signal"], Coverage(shots=["150"]), stage=1)
    with pytest.raises(FDSValidationError):
        make_cal_version("b", ["signal"], Coverage(shots=["150"]), stage=1)
...
def test_chain_includes_only_covering_stages(calibration, make_cal_version, get_shot):
    ...
    absolute = make_cal_version(
        "absolute", ["signal"], Coverage(shots=["150"]), stage=2
    )
